...
echo "********  Building Global Domination Android native libraries"
( cd ../../../builds/android && ./build.sh )

#   Ensure we've built JNI interface
echo "********  Building Global Domination JNI interface & classes"
( cd .. && gradle build jar )
...
echo "********  Building Global Domination JNI for Android"
make $MAKE_OPTIONS

echo "********  Building global_domination.jar for Android"
#   Copy class files into org/zeromq/etc.
unzip -q ../../build/libs/gdom*.jar

#   Copy native libraries into lib/armeabi
mkdir -p lib/armeabi
mv libgdomjni.so lib/armeabi
...
zip -r -m ../gdom-android.jar lib/ org/ META-INF/
cd ..
rm -rf build
